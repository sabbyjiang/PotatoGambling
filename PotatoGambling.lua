...
Admin:SetSize(112, 120) 
Admin:SetPoint("BOTTOMLEFT", GUI, "TOPLEFT", -111, -120)
SetTemplate(Admin)
Admin:Hide()
...
Admin2:SetSize(112, 120)
Admin2:SetPoint("BOTTOMLEFT", GUI, "TOPRIGHT", 0, -120)
SetTemplate(Admin2)
Admin2:Hide()
...
GameMode_Button = CreateFrame("Frame", nil, Admin2)
GameMode_Button:SetSize(112, 10)
GameMode_Button:SetPoint("CENTER", Admin2Top, "CENTER", 0, -1)
...
GameMode_Button_Button:SetText("< Munty's Casino >")
...
CrossGambling_EditBox2:EnableKeyboard(true)
CrossGambling_EditBox2:EnableMouse(true)
...
CrossGambling_HouseCut:SetPoint("TOPLEFT", Admin2, 3, -98)
...
function CrossGambling_SlashCmd(msg)
	...
	if (string.sub(msg, 1, 6) == "remove") then 
		PotatoGambling_RemoveApprovedMember(strsub(msg, 8));
		msgPrint = 1;
	end
	if (string.sub(msg, 1, 3) == "add") then 
		PotatoGambling_AddApprovedMember(strsub(msg, 5));
		msgPrint = 1;
	end
...
function CrossGambling_OnEvent(self, event, ...)
	...
	if (event == "PLAYER_ENTERING_WORLD") then
		...
		CrossGambling_EditBox2:SetText(""..CrossGambling["GameMode1"]);
	...
	if ((event == "CHAT_MSG_RAID_LEADER" or event == "CHAT_MSG_RAID") and CrossGambling["chat"] == 1) then
		local msg, _,_,_,name = ... -- name no realm
		PotatoGambling_GetStat(msg, name)

	elseif ((event == "CHAT_MSG_PARTY_LEADER" or event == "CHAT_MSG_PARTY") and CrossGambling["chat"] == 2) then
		local msg, name = ... -- name no realm
		PotatoGambling_GetStat(msg, name)
	
	elseif (event == "CHAT_MSG_GUILD" and CrossGambling["chat"] == 3) then
		local msg, name = ... -- name no realm
		PotatoGambling_GetStat(msg, name)
	
	elseif event == "CHAT_MSG_CHANNEL" and CrossGambling["chat"] == 4 then
		local msg,_,_,_,name,_,_,_,channelName = ...
		if channelName == CrossGambling["channel"] then
			PotatoGambling_GetStat(msg, name)
		end
...
if CrossGambling_EditBox2:GetText() == "501" then
 ...
 GameMode_Button_Button:SetText("< Munty's Casino >");
  CrossGambling_EditBox2:SetText("501");
 elseif CrossGambling_EditBox2:GetText() > "" then
 GameMode_Button_Button:SetText("< Munty's Casino >");
  CrossGambling_EditBox2:SetText("501");
end
end
...
function PotatoGambling_RemoveApprovedMember(name)
	...
	if (normalizedName ~= nil and normalizedName ~= "") then
		...
		Print("", "", string.format("|cffffff00Error: No approved member found by name %s.", normalizedName));
	else
		Print("", "", "|cffffff00Error: No name provided.");
	end
end
